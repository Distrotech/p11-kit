...
#define RPC_CLIENT_INIT(id, function_list) \
              ...
              int call_id)
{
	...
	buffer = p11_buffer_new_full (64, log_allocator, free);
	return_val_if_fail (buffer != NULL, CKR_GENERAL_ERROR);

	/* We use the same buffer for reading and writing */
	_p11_rpc_message_init (msg, buffer, _p11_buffer_to_output (buffer));
          ...
          RpcMessage *msg)
{
	...
	if (_p11_buffer_failed (msg->output))
	...
	if (!_p11_rpc_message_parse (msg, RPC_RESPONSE))
		return CKR_DEVICE_ERROR;

	/* If it's an error code then return it */
	if (msg->call_id == RPC_CALL_ERROR) {
		if (!_p11_rpc_message_read_ulong (msg, &ckerr)) {
			_p11_message ("invalid rpc error response: too short");
			return CKR_DEVICE_ERROR;
		}

		if (ckerr <= CKR_OK) {
			_p11_message ("invalid rpc error response: bad error code");
			return CKR_DEVICE_ERROR;
		}
	...
	if (call_id != msg->call_id) {
		_p11_message ("invalid rpc response: call mismatch");
		return CKR_DEVICE_ERROR;
	}

	assert (!_p11_buffer_failed (msg->input));
           ...
           CK_RV ret)
{
	...
	if (ret == CKR_OK) {
		if (_p11_buffer_failed (msg->input)) {
			_p11_message ("invalid rpc response: bad argument data");
	...
	assert (msg->input == &msg->output->buf);
	p11_buffer_free (msg->input);
                            ...
                            CK_ULONG len)
{
	...
	if (!_p11_buffer_get_uint32 (msg->input, msg->parsed, &msg->parsed, &num))
		return PARSE_ERROR;
	...
	for (i = 0; i < num; ++i) {
		...
		/* The attribute type */
		_p11_buffer_get_uint32 (msg->input, msg->parsed, &msg->parsed, &type);

		/* Attribute validity */
		_p11_buffer_get_byte (msg->input, msg->parsed, &msg->parsed, &validity);

		/* And the data itself */
		if (validity) {
			if (_p11_buffer_get_uint32 (msg->input, msg->parsed, &msg->parsed, &value) &&
			    _p11_buffer_get_byte_array (msg->input, msg->parsed, &msg->parsed, &attrval, &attrlen)) {
                       ...
                       CK_ULONG max)
{
	...
	if (!_p11_buffer_get_byte (msg->input, msg->parsed, &msg->parsed, &valid))
		return PARSE_ERROR;

	/* If not valid, then just the length is encoded, this can signify CKR_BUFFER_TOO_SMALL */
	if (!valid) {
		if (!_p11_buffer_get_uint32 (msg->input, msg->parsed, &msg->parsed, &length))
			return PARSE_ERROR;
	...
	if (!_p11_buffer_get_byte_array (msg->input, msg->parsed, &msg->parsed, &val, &vlen))
		return PARSE_ERROR;
                        ...
                        CK_ULONG_PTR len, CK_ULONG max)
{
	...
	*len = num;
	...
	/* We need to go ahead and read everything in all cases */
	for (i = 0; i < num; ++i) {
		_p11_buffer_get_uint64 (msg->input, msg->parsed, &msg->parsed, &val);
		if (arr)
			arr[i] = (CK_ULONG)val;
	}

	return _p11_buffer_failed (msg->input) ? PARSE_ERROR : CKR_OK;
}

/* Used to override the supported mechanisms in tests */
CK_MECHANISM_TYPE *_p11_rpc_mechanisms_override_supported = NULL;
...
mechanism_has_sane_parameters (CK_MECHANISM_TYPE type)
{
	...
	if (_p11_rpc_mechanisms_override_supported) {
		for (i = 0; _p11_rpc_mechanisms_override_supported[i] != 0; i++) {
			if (_p11_rpc_mechanisms_override_supported[i] == type)
				return 1;
		}
                       ...
                       CK_MECHANISM_PTR mech)
{
	...
	/* The mechanism type */
	_p11_buffer_add_uint32 (msg->output, mech->mechanism);
	...
	if (mechanism_has_no_parameters (mech->mechanism))
		_p11_buffer_add_byte_array (msg->output, NULL, 0);
	else if (mechanism_has_sane_parameters (mech->mechanism))
		_p11_buffer_add_byte_array (msg->output, mech->pParameter,
		                            mech->ulParameterLen);
	else
		return CKR_MECHANISM_INVALID;

	return _p11_buffer_failed (msg->output) ? CKR_HOST_MEMORY : CKR_OK;
}
...
#define BEGIN_CALL_OR(call_id, module, if_no_daemon) \
	...
	{ \
		RpcClient *_mod = module; RpcMessage _msg; \
...
static CK_RV
rpc_C_Initialize (RpcClient *module,
                  CK_VOID_PTR init_args)
{
	...
	_p11_debug ("C_Initialize: %lu", ret);
	return ret;
}

static CK_RV
rpc_C_Finalize (RpcClient *module,
                CK_VOID_PTR reserved)
{
	...
	if (module->initialize_done) {
		...
		call_done (module, &msg, ret);
		if (ret != CKR_OK)
			_p11_message ("finalizing the daemon returned an error: %lu", ret);
	...
	_p11_debug ("C_Finalize: %lu", CKR_OK);
	return CKR_OK;
}
...
static CK_RV
rpc_C_GetInfo (RpcClient *module,
               CK_INFO_PTR info)
{
...
static CK_RV
rpc_C_GetFunctionList (RpcClient *module,
...
static CK_RV
rpc_C_GetSlotList (RpcClient *module,
...
static CK_RV
rpc_C_GetSlotInfo (RpcClient *module,
                   CK_SLOT_ID slot_id,
                   CK_SLOT_INFO_PTR info)
{
	return_val_if_fail (info, CKR_ARGUMENTS_BAD);
...
static CK_RV
rpc_C_GetTokenInfo (RpcClient *module,
                    CK_SLOT_ID slot_id,
                    CK_TOKEN_INFO_PTR info)
{
	return_val_if_fail (info, CKR_ARGUMENTS_BAD);
...
static CK_RV
rpc_C_GetMechanismList (RpcClient *module,
                        CK_SLOT_ID slot_id,
...
static CK_RV
rpc_C_GetMechanismInfo (RpcClient *module,
                        CK_SLOT_ID slot_id,
...
static CK_RV
rpc_C_InitToken (RpcClient *module,
                 CK_SLOT_ID slot_id,
...
static CK_RV
rpc_C_WaitForSlotEvent (RpcClient *module,
                        CK_FLAGS flags,
                        CK_SLOT_ID_PTR slot,
                        CK_VOID_PTR reserved)
{
...
static CK_RV
rpc_C_OpenSession (RpcClient *module,
                   CK_SLOT_ID slot_id,
                   CK_FLAGS flags,
...
static CK_RV
rpc_C_CloseSession (RpcClient *module,
...
static CK_RV
rpc_C_CloseAllSessions (RpcClient *module,
                        CK_SLOT_ID slot_id)
{
...
static CK_RV
rpc_C_GetFunctionStatus (RpcClient *module,
                         CK_SESSION_HANDLE session)
{
...
static CK_RV
rpc_C_CancelFunction (RpcClient *module,
                      CK_SESSION_HANDLE session)
{
...
static CK_RV
rpc_C_GetSessionInfo (RpcClient *module,
                      CK_SESSION_HANDLE session,
                      CK_SESSION_INFO_PTR info)
{
	return_val_if_fail (info, CKR_ARGUMENTS_BAD);
...
static CK_RV
rpc_C_InitPIN (RpcClient *module,
               CK_SESSION_HANDLE session,
               CK_UTF8CHAR_PTR pin,
               CK_ULONG pin_len)
{
...
static CK_RV
rpc_C_GetOperationState (RpcClient *module,
                         CK_SESSION_HANDLE session,
...
static CK_RV
rpc_C_SetOperationState (RpcClient *module,
                         CK_SESSION_HANDLE session,
                         CK_BYTE_PTR operation_state,
...
static CK_RV
rpc_C_CreateObject (RpcClient *module,
                    CK_SESSION_HANDLE session,
...
static CK_RV
rpc_C_CopyObject (RpcClient *module,
                  CK_SESSION_HANDLE session,
...
static CK_RV
rpc_C_DestroyObject (RpcClient *module,
                     CK_SESSION_HANDLE session,
                     CK_OBJECT_HANDLE object)
{
...
static CK_RV
rpc_C_GetObjectSize (RpcClient *module,
                     CK_SESSION_HANDLE session,
                     CK_OBJECT_HANDLE object,
                     CK_ULONG_PTR size)
{
...
static CK_RV
rpc_C_GetAttributeValue (RpcClient *module,
                         CK_SESSION_HANDLE session,
                         CK_OBJECT_HANDLE object,
                         CK_ATTRIBUTE_PTR template,
                         CK_ULONG count)
...
static CK_RV
rpc_C_SetAttributeValue (RpcClient *module,
                         CK_SESSION_HANDLE session,
                         CK_OBJECT_HANDLE object,
                         CK_ATTRIBUTE_PTR template,
                         CK_ULONG count)
...
static CK_RV
rpc_C_FindObjectsInit (RpcClient *module,
                       CK_SESSION_HANDLE session,
                       CK_ATTRIBUTE_PTR template,
                       CK_ULONG count)
{
...
static CK_RV
rpc_C_FindObjects (RpcClient *module,
                   CK_SESSION_HANDLE session,
...
static CK_RV
rpc_C_FindObjectsFinal (RpcClient *module,
                        CK_SESSION_HANDLE session)
{
...
static CK_RV
rpc_C_EncryptInit (RpcClient *module,
                   CK_SESSION_HANDLE session,
...
static CK_RV
rpc_C_Encrypt (RpcClient *module,
               CK_SESSION_HANDLE session,
               CK_BYTE_PTR data,
               CK_ULONG data_len,
...
static CK_RV
rpc_C_EncryptUpdate (RpcClient *module,
                     CK_SESSION_HANDLE session,
                     CK_BYTE_PTR part,
                     CK_ULONG part_len,
...
static CK_RV
rpc_C_EncryptFinal (RpcClient *module,
                    CK_SESSION_HANDLE session,
...
static CK_RV
rpc_C_DecryptInit (RpcClient *module,
                   CK_SESSION_HANDLE session,
                   CK_MECHANISM_PTR mechanism,
                   CK_OBJECT_HANDLE key)
{
...
static CK_RV
rpc_C_Decrypt (RpcClient *module,
               CK_SESSION_HANDLE session,
...
static CK_RV
rpc_C_DecryptUpdate (RpcClient *module,
                     CK_SESSION_HANDLE session,
...
static CK_RV
rpc_C_DecryptFinal (RpcClient *module,
                    CK_SESSION_HANDLE session,
                    CK_BYTE_PTR last_part,
                    CK_ULONG_PTR last_part_len)
{
...
static CK_RV
rpc_C_DigestInit (RpcClient *module,
                  CK_SESSION_HANDLE session,
                  CK_MECHANISM_PTR mechanism)
{
...
static CK_RV
rpc_C_DigestUpdate (RpcClient *module,
                    CK_SESSION_HANDLE session,
                    CK_BYTE_PTR part,
                    CK_ULONG part_len)
{
...
static CK_RV
rpc_C_DigestKey (RpcClient *module,
                 CK_SESSION_HANDLE session,
                 CK_OBJECT_HANDLE key)
{
...
static CK_RV
rpc_C_DigestFinal (RpcClient *module,
                   CK_SESSION_HANDLE session,
                   CK_BYTE_PTR digest,
                   CK_ULONG_PTR digest_len)
{
...
static CK_RV
rpc_C_SignInit (RpcClient *module,
                CK_SESSION_HANDLE session,
                CK_MECHANISM_PTR mechanism,
                CK_OBJECT_HANDLE key)
{
...
static CK_RV
rpc_C_SignUpdate (RpcClient *module,
                  CK_SESSION_HANDLE session,
                  CK_BYTE_PTR part,
                  CK_ULONG part_len)
{
...
static CK_RV
rpc_C_SignFinal (RpcClient *module,
                 CK_SESSION_HANDLE session,
                 CK_BYTE_PTR signature,
                 CK_ULONG_PTR signature_len)
{
...
static CK_RV
rpc_C_SignRecoverInit (RpcClient *module,
                       CK_SESSION_HANDLE session,
                       CK_MECHANISM_PTR mechanism,
                       CK_OBJECT_HANDLE key)
{
...
static CK_RV
rpc_C_SignRecover (RpcClient *module,
                   CK_SESSION_HANDLE session,
                   CK_BYTE_PTR data,
                   CK_ULONG data_len,
...
static CK_RV
rpc_C_VerifyInit (RpcClient *module,
                  CK_SESSION_HANDLE session,
                  CK_MECHANISM_PTR mechanism,
                  CK_OBJECT_HANDLE key)
{
...
static CK_RV
rpc_C_VerifyUpdate (RpcClient *module,
                    CK_SESSION_HANDLE session,
                    CK_BYTE_PTR part,
                    CK_ULONG part_len)
{
...
static CK_RV
rpc_C_VerifyFinal (RpcClient *module,
                   CK_SESSION_HANDLE session,
                   CK_BYTE_PTR signature,
                   CK_ULONG signature_len)
{
...
static CK_RV
rpc_C_VerifyRecoverInit (RpcClient *module,
                         CK_SESSION_HANDLE session,
                         CK_MECHANISM_PTR mechanism,
                         CK_OBJECT_HANDLE key)
{
...
static CK_RV
rpc_C_VerifyRecover (RpcClient *module,
                     CK_SESSION_HANDLE session,
                     CK_BYTE_PTR signature,
                     CK_ULONG signature_len,
                     CK_BYTE_PTR data,
...
static CK_RV
rpc_C_DigestEncryptUpdate (RpcClient *module,
                           CK_SESSION_HANDLE session,
                           CK_BYTE_PTR part,
                           CK_ULONG part_len,
                           CK_BYTE_PTR enc_part,
...
static CK_RV
rpc_C_DecryptDigestUpdate (RpcClient *module,
                           CK_SESSION_HANDLE session,
                           CK_BYTE_PTR enc_part,
                           CK_ULONG enc_part_len,
                           CK_BYTE_PTR part,
...
static CK_RV
rpc_C_SignEncryptUpdate (RpcClient *module,
                         CK_SESSION_HANDLE session,
                         CK_BYTE_PTR part,
                         CK_ULONG part_len,
                         CK_BYTE_PTR enc_part,
...
static CK_RV
rpc_C_DecryptVerifyUpdate (RpcClient *module,
                           CK_SESSION_HANDLE session,
                           CK_BYTE_PTR enc_part,
                           CK_ULONG enc_part_len,
                           CK_BYTE_PTR part,
...
static CK_RV
rpc_C_GenerateKey (RpcClient *module,
                   CK_SESSION_HANDLE session,
                   CK_MECHANISM_PTR mechanism,
                   CK_ATTRIBUTE_PTR template,
                   CK_ULONG count,
...
static CK_RV
rpc_C_GenerateKeyPair (RpcClient *module,
                       CK_SESSION_HANDLE session,
                       CK_MECHANISM_PTR mechanism,
...
static CK_RV
rpc_C_WrapKey (RpcClient *module,
               CK_SESSION_HANDLE session,
               CK_MECHANISM_PTR mechanism,
...
static CK_RV
rpc_C_UnwrapKey (RpcClient *module,
                 CK_SESSION_HANDLE session,
                 CK_MECHANISM_PTR mechanism,
...
static CK_RV
rpc_C_DeriveKey (RpcClient *module,
                 CK_SESSION_HANDLE session,
                 CK_MECHANISM_PTR mechanism,
...
static CK_RV
rpc_C_SeedRandom (RpcClient *module,
                  CK_SESSION_HANDLE session,
                  CK_BYTE_PTR seed,
                  CK_ULONG seed_len)
{
...
static CK_RV
rpc_C_GenerateRandom (RpcClient *module,
                      CK_SESSION_HANDLE session,
...
/*
 * This macro defines an RPC PKCS#11 client module.
 *
 ...
 * RpcClient to the actual implementation functions above.
 */

#define RPC_DEFINE_CLIENT(id) \
	static RpcClient rpc_module_##id; \
	...
	static RpcClient rpc_module_##id = RPC_CLIENT_INIT (id, &rpc_function_list##id);

/*
 * This macro is to save typing in the two definitions below.
 * RPC_CLIENT is redefined as needed below. The numbers must be sequential.
 */
#define RPC_CLIENT_ELEMENTS \
	RPC_CLIENT (0) RPC_CLIENT (1) RPC_CLIENT (2) RPC_CLIENT (3) RPC_CLIENT (4) \
	RPC_CLIENT (5) RPC_CLIENT (6) RPC_CLIENT (7) RPC_CLIENT (8) RPC_CLIENT (9) \
	RPC_CLIENT (10) RPC_CLIENT (11) RPC_CLIENT (12) RPC_CLIENT (13) RPC_CLIENT (14) \
	RPC_CLIENT (15) RPC_CLIENT (16) RPC_CLIENT (17) RPC_CLIENT (18) RPC_CLIENT (19) \
	RPC_CLIENT (20) RPC_CLIENT (21) RPC_CLIENT (22) RPC_CLIENT (23) RPC_CLIENT (24) \
	RPC_CLIENT (25) RPC_CLIENT (26) RPC_CLIENT (27) RPC_CLIENT (28) RPC_CLIENT (29) \
	RPC_CLIENT (30) RPC_CLIENT (31) RPC_CLIENT (32) RPC_CLIENT (33) RPC_CLIENT (34) \
	RPC_CLIENT (35) RPC_CLIENT (36) RPC_CLIENT (37) RPC_CLIENT (38) RPC_CLIENT (39) \
	RPC_CLIENT (40) RPC_CLIENT (41) RPC_CLIENT (42) RPC_CLIENT (43) RPC_CLIENT (44) \
	RPC_CLIENT (45) RPC_CLIENT (46) RPC_CLIENT (47) RPC_CLIENT (48) RPC_CLIENT (49) \
	RPC_CLIENT (50) RPC_CLIENT (51) RPC_CLIENT (52) RPC_CLIENT (53) RPC_CLIENT (54) \
	RPC_CLIENT (55) RPC_CLIENT (56) RPC_CLIENT (57) RPC_CLIENT (58) RPC_CLIENT (59) \
	RPC_CLIENT (60) RPC_CLIENT (61) RPC_CLIENT (62) RPC_CLIENT (63)

/*
 * Use RPC_DEFINE_MODULE to define all the RPC functions and function lists
 * for each module.
 */
#define RPC_CLIENT(id) RPC_DEFINE_CLIENT (id)
RPC_CLIENT_ELEMENTS
#undef RPC_CLIENT

/* Now init the array of modules */
static RpcClient *RPC_CLIENTS[] = {
	#define RPC_CLIENT(id) &rpc_module_##id,
	RPC_CLIENT_ELEMENTS
	#undef RPC_CLIENT
};

#define N_RPC_CLIENTS \
	(sizeof (RPC_CLIENTS) / sizeof (RPC_CLIENTS[0]))

CK_FUNCTION_LIST_PTR
p11_rpc_client_register (P11RpcClientVtable *vtable)
{
	...
	return_val_if_fail (vtable != NULL, NULL);
	return_val_if_fail (vtable->connect != NULL, NULL);
	return_val_if_fail (vtable->transport != NULL, NULL);
	return_val_if_fail (vtable->disconnect != NULL, NULL);
	...
	for (i = 0; i < N_RPC_CLIENTS; i++) {
		assert (RPC_CLIENTS[i]->check == i);
		if (RPC_CLIENTS[i]->vtable == NULL) {
			RPC_CLIENTS[i]->vtable = vtable;
			function_list = RPC_CLIENTS[i]->function_list;
	...
	if (function_list == NULL)
		_p11_message ("too many rpc client modules: %d", (int)N_RPC_CLIENTS);
